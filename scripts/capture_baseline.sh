#!/usr/bin/env bash
# C5: capture the naive FP16 serving baseline against a RUNNING real endpoint.
# Assumes 'make serve' is up on $KISAN_SERVING_URL. Writes baseline.json (canonical,
# concurrency=1) + l3_1_fp16.json, plus a 1/4/8 concurrency sweep for L3.3 to beat.
set -euo pipefail
...
uv run python -m eval.perf.bench --label fp16 --concurrency 1 --n "$N" \
  --max-tokens "$MAXTOK" --endpoint "$URL" --out eval/results/baseline.json
cp eval/results/baseline.json eval/results/l3_1_fp16.json

# Concurrency sweep (L3.3 batching reference curve).
for c in 4 8; do
  uv run python -m eval.perf.bench --label "fp16_c${c}" --concurrency "$c" --n "$N" \
    --max-tokens "$MAXTOK" --endpoint "$URL" --out "eval/results/fp16_c${c}.json"
done

echo ""
echo "=== wrote ==="
ls -1 eval/results/baseline.json eval/results/l3_1_fp16.json eval/results/fp16_c*.json
echo ""
